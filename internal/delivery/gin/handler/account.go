...
func (h *Handler) CreateAccount(ctx *gin.Context) {
	...
	if a, err := h.createAccount.Execute(ctx, &app.CreateAccountParams{
...
func (h *Handler) GetAccount(ctx *gin.Context) {
	...
	authPayload := ctx.MustGet(helper.AuthorizationPayloadKey).(*token.Payload)
	if a, err := h.getAccount.Execute(ctx, &app.GetAccountsParams{
		ID:    req.ID,
		Owner: authPayload.Username,
	}); err != nil {
		ctx.Error(err)
...
func (h *Handler) ListAccounts(ctx *gin.Context) {
	...
	authPayload := ctx.MustGet(helper.AuthorizationPayloadKey).(*token.Payload)
	if as, err := h.listAccounts.Execute(ctx, &app.ListAccountsParams{
		Owner:  authPayload.Username,
		Limit:  req.PageSize,
