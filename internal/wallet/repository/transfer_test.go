...
func TestCreateTransferDeadlock(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func() {
			t := wallet.NewTransfer(fromAccountID, toAccountID, amount)
...
func createRandomTransfer(t *testing.T, account1, account2 *wallet.Account) *wallet.Transfer {
	args := wallet.NewTransfer(account1.ID, account2.ID, wallet.RandomMoney())
...
func TestListTransfer(t *testing.T) {
	...
	arg := &wallet.ListTransfersParams{
		FromAccountID: account1.ID,
		ToAccountID:   account1.ID,
		Limit:         5,
		Offset:        5,
