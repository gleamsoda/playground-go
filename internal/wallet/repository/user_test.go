...
func createRandomUser(t *testing.T) *wallet.User {
	hashedPassword, err := password.Hash(wallet.RandomString(6))
	require.NoError(t, err)

	arg := &wallet.User{
		Username:       wallet.RandomOwner(),
		HashedPassword: hashedPassword,
		FullName:       wallet.RandomOwner(),
		Email:          wallet.RandomEmail(),
	}
...
func TestUpdateUserOnlyFullName(t *testing.T) {
	...
	newFullName := wallet.RandomOwner()
...
func TestUpdateUserOnlyPassword(t *testing.T) {
	...
	newPassword := wallet.RandomString(6)
